...
interface BlogDetailModalProps {
  post: Post;
  onClose: () => void;
  onUpdate: (postId: string, updatedData: Partial<Post>, successMessage?: string) => Promise<boolean>;
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto overscroll-contain border-2 border-black">
        <div className="flex justify-between items-start mb-6">
            <div className="flex-1 pr-4">
                ...
                <div className="flex items-center gap-4">
                  <span className={`px-3 py-1 text-sm font-bold rounded-full ${getStatusClasses(post.status)}`}>
                    {post.status}
                  </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg border overflow-y-auto overscroll-contain" style={{ maxHeight: '16rem' }}>                  <div className="text-sm leading-relaxed space-y-3">
                    {post.body.split('\n').filter(line => line.trim() !== '').slice(0, 5).map((paragraph, index) => (
                      <p key={index} className="text-gray-700">
                        {paragraph.trim()}
                      </p>
                    ))}
                    {post.body.split('\n').filter(line => line.trim() !== '').length > 5 && (
                      <p className="text-gray-500 italic">... (content continues)</p>
                    )}
                  </div>
                </div>
                ...
                {/* Toasty Take */}
                {post.toasty_take && (
                  <div className="bg-tst-green p-4 rounded-lg border-2 border-black overflow-y-auto overscroll-contain" style={{ maxHeight: '8rem' }}>
            ...
            <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                    ...
                    <div>
                        ...
                        <span className={`inline-block px-2 py-1 text-xs font-bold rounded-full ${getStatusClasses(post.status)}`}>
                            {post.status}
                        </span>
                    </div>
                    ...
                    {post.slug && (
                        <div>
                            ...
                            <p className="text-xs font-mono bg-gray-200 p-1 rounded">/posts/{post.slug}</p>
                        </div>
                    )}

                    <div>
                        <p className="text-sm font-bold text-gray-600">Public Visibility</p>
                        <span className={`inline-block px-2 py-1 text-xs font-bold rounded-full ${
                            post.visible_to_public ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
                ...
                {post.archive_posts && post.archive_posts.length > 0 && (
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      {post.archive_posts.length} related post{post.archive_posts.length !== 1 ? 's' : ''} selected
                    </div>
                  </div>
                )}
            </div>
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-200">
            <div className="flex gap-3">
                ...
                {post.status === 'published' && post.slug && post.visible_to_public && !post.archived && (
                  <Button
                ...
                {post.status === 'published' && !post.archived && (
                  <Button
                      onClick={() => handleUpdatePost(post.id, { visible_to_public: !post.visible_to_public })}
                      disabled={isArchiving || isUnarchiving}
                  ...
                  >
                      {post.visible_to_public ? '👁️ Hide from Public' : '👁️ Make Public'}
                  </Button>
                )}
            ...
            <div className="text-sm text-gray-500">
              {post.archived ? 'Archived posts are hidden from public view' : 'Active posts are visible to the public'}
            </div>
        </div>
      </div>
    </div>
