...
export async function POST(request: Request) {
  try {
    ...
    const { data: archivePosts, error: archiveError } = await supabase.from('posts').select('title, image_url, slug, subtext').in('id', postData.archive_posts || []);
    if (archiveError) throw new Error(`Error fetching archive posts: ${archiveError.message}`);

    const formattedDate = new Intl.DateTimeFormat('en-US', { month: 'long', year: 'numeric' }).format(new Date());

    const emailData = {
      header_title: postData.title || 'A note on healing',
      formatted_date: formattedDate,
      main_image_url: postData.image_url || 'https://placehold.co/640x360/F9F5F2/000000?text=Main+Article+Image',
    ...
    return NextResponse.json({ error: `Preview generation failed: ${errorMessage}` }, { status: 500 });
  }
}
