...
describe('/api/contact API Route', () => {
  describe('Input Validation', () => {
    ...
    it('should validate required email', () => {
      ...
      expect(() => validateContactData({ name: 'John' })).toThrow(
        'Email is required.'
      );
    });

    it('should validate required name', () => {
      expect(() => validateContactData({ email: 'test@example.com' })).toThrow(
        'Name is required.'
      );
    });
    ...
    it('should validate email format', () => {
      expect(() =>
        validateContactData({ name: 'John', email: 'invalid-email' })
      ).toThrow('Invalid email format.');
      expect(() =>
        validateContactData({ name: 'John', email: 'test@' })
      ).toThrow('Invalid email format.');
      expect(() =>
        validateContactData({ name: 'John', email: '@example.com' })
      ).toThrow('Invalid email format.');
    });

    it('should validate name length', () => {
      expect(() =>
        validateContactData({ name: 'J', email: 'test@example.com' })
  ...
  describe('Database Operations', () => {
    ...
    it('should handle database errors', async () => {
      ...
      await expect(insertContact(contactData)).rejects.toThrow(
  ...
  describe('Security and Rate Limiting', () => {
    ...
    it('should handle extremely long inputs', () => {
      const validateInput = (data: any) => {
        if (data.name && data.name.length > 1000)
