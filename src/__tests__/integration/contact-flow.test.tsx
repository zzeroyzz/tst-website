...
jest.mock('@/components/CircleIcon/CircleIcon', () => {
  return function MockCircleIcon({ size, bgColor, iconUrl }: any) {
    return (
      <div className={`circle-icon ${bgColor}`} data-size={size}>
        Icon
      </div>
    );
  };
...
describe('Contact Flow Integration', () => {
  ...
  it('completes full contact submission flow', async () => {
    ...
    expect(
      screen.getByText('A Space to Be Seen and Heard')
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Here's what to expect next:/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/You'll receive a personal email from me/)
      ).toBeInTheDocument();
    });
  });
  ...
  it('shows benefit cards with correct content', () => {
    ...
    expect(
      screen.getByText('Start Where You Are, Not Where You Think You Should Be')
    ).toBeInTheDocument();
    expect(screen.getByText('A Space Free of Judgment')).toBeInTheDocument();
    expect(
      screen.getByText('One Conversation, Zero Commitment')
    ).toBeInTheDocument();
  });
