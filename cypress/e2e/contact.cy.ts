describe('Contact Page', () => {
  ...
  it('should allow filling out and submitting the contact form', () => {
    ...
    cy.get('input[placeholder*="Your name"]').filter(':visible').first().type('Test User')
    cy.get('input[type="email"]').filter(':visible').first().type('test@example.com')
    cy.get('input[placeholder*="Phone number"]').filter(':visible').first().type('4044042244')

    // Submit form
    cy.get('form').filter(':visible').first().within(() => {
