...
async function runAllTests() {
  ...
  for (const suiteKey of testOrder) {
    ...
    if (!results[suiteKey] && !process.argv.includes('--continue')) {
      log(
        '\n🛑 Stopping on first failure. Use --continue to run all tests.',
        'yellow'
      );
      break;
    }
...
switch (command) {
  ...
  case 'search':
    if (args[1]) {
      ...
      log(
        '❌ Please provide a test pattern: npm run test:search "pattern"',
        'red'
      );
      process.exit(1);
    }
  ...
  case '-h':
    ...
    log(
      'npm run test:runner search <pattern> Run tests matching pattern',
      'blue'
    );
    log('');
    log('Flags:', 'yellow');
